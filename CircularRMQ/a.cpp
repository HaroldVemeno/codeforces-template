...
// clopen interval
ll go(int l, int r, int add = 0, int lu = 0, int ru = nn, int ui = 1) {
    D(l)
    D(r)
    D(lu)
    D(ru)
    if(ru <= l || r <= lu || r < l || lu >= n) return M;
    if(un[ui].la != 0) {
        if(ui <= nn) {
            un[ui*2].la += un[ui].la;
            un[ui*2].m += un[ui].la;
    ...
    ll m1 = go(l, r, add, lu, (lu+ru)/2, ui*2);
    ll m2 = go(l, r, add, (lu+ru)/2, ru, ui*2+1);
